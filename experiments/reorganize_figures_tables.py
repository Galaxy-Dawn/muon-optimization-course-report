...
the main paper. Figures use matplotlib with pubfig export defaults; tables are
written as LaTeX fragments with booktabs and light cell highlighting.
"""
import csv
...
METHOD_LABELS = {
    ...
    "muonw": "MuonW",
    "rms_muonw": "RMS-MuonW",
    "newton_muonw": "Newton-MuonW",
}
METHOD_ORDER = ["sgd", "adam", "adamw", "muon", "muonw", "rms_muonw", "newton_muonw"]
LOGREG_ORDER = ["sgd", "adam", "adamw", "muon", "muonw", "rms_muonw", "newton_muonw"]
PALETTE = {
    "baseline": "#4C78A8",
    "precond": "#59A14F",
    "muon": "#B279A2",
...
def make_classification_figure(base_name="classification_diagnostics_composite", add_labels=False):
    ...
    ax = axes[1, 0]
    log_methods = ["sgd", "adamw", "muon", "muonw", "rms_muonw", "newton_muonw"]
    histories = {m: logreg_module.run("gd" if m == "sgd" else m, seed=42, record=True) for m in log_methods}
    pf.line(
        np.column_stack([histories[m]["loss"] for m in log_methods]),
        x=np.arange(len(next(iter(histories.values()))["loss"])),
    ...
    ax = axes[1, 1]
    mlp_methods = ["sgd", "adamw", "muonw", "rms_muonw", "newton_muonw"]
    ...
    pf.line(
        ...
        x=np.arange(len(next(iter(histories.values()))["max_logit"])),
        ...
        for task, order, fixed in [
            ...
            for method in order:
                ...
                if task == "mlp":
                    ...
                    elif method in ["muonw", "newton_muonw"]:
